...
Coordinators run binding checks at every start/resume; delegated workers and reviewers use their supplied assignment instead. Bootstrap missing state yourself; user configuration is optional. Resolve tool names, schemas, pagination, and supported operations from live Doska MCP instructions before calling them.

## Binding scope

- **Persistent binding:** `.taskboard/settings.md` selects the project default. Treat `server`, `board_name`, `board_id`, `charter_id`, and `setup_state` as one binding; validate them together before persisting a change. Preserve notes and unrelated fields.
...
- Change the default only when the user requests a project-binding change. Verify the target first, then replace the binding fields together; never carry the old board’s charter or initialization authority across. Persist temporary initialization intent in a board-qualified pending record when initialization is authorized.

All settings writes below apply to the persistent binding only. For a temporary override, use its separate session/recovery record and verify it against that board; never reconcile records from another binding into it.

## 1. Locate and initialize local settings

Use the repository root, not the current subdirectory or skill installation directory. For a non-Git project, use the workspace project root. In a multi-root workspace, select the root containing the requested work; ask only if the scope remains ambiguous. Branches/worktrees reuse the same binding. If settings are absent, inspect the main checkout and the shared setup journal described below before planning creation. Copy a verified existing binding; conflicting copies require reconciliation. Repository identity is documentation, not a board-discovery mechanism.
...
3. On every persistent binding update, save the binding fields and each setup-state transition to the shared journal before the checkout settings and before its corresponding remote operation. Use atomic file replacement for each local write. Immediately journal returned IDs before copying them to settings. Record the prior binding fields and the intended replacement so an interrupted copy can be distinguished from independent edits: if settings match the recorded prior fields, finish the copy; otherwise reconcile rather than overwrite. Deliberate binding changes must be recorded as such with the old and new identities. Uncertain remote creation follows Interrupted creation below.
...
The local claim coordinates only processes sharing that filesystem. Separate clones or machines must use an explicitly shared binding/initializer agreement; local locking cannot guarantee uniqueness across them. If no shared claim can be acquired, defer new-board creation and binding mutations. Use an existing explicit board under Recovery where safe. A temporary override does not acquire or modify the default’s bootstrap state; shared-board initialization still requires agreement on its writer.
...
- Keep settings suitable for version control, subject to repository policy; sanitize remote credentials and omit tokens and task contents. Do not commit files automatically. Treat local recovery records as potentially sensitive and exclude `.taskboard/pending/` and non-Git local lock/journal files through the project’s existing local-ignore mechanism when available.

**Complete when:** default settings exist with a project identity and board name (or an explicit temporary binding is recorded separately), existing bindings and notes are preserved, and any bootstrap writer is established. If local writes are unavailable, report the constraint and use the recovery rules below.
...
**Complete when:** exactly one intended board is readable and its verified effective binding is recorded without modifying an unrelated default. If persistence or server access is blocked, report remote-only or local-only tracking, or a conversation-only blocked handoff, as specified under Recovery.

## 3. Establish the working agreement

### Agent-created board or authorized initialization

Use this path only for a board just created by this setup or whose persisted `setup_state: initializing` and provenance authorize continuing it. Confirm one shared-structure writer before any initialization mutations; a local bootstrap claim alone does not coordinate other clones or remote actors. A pre-existing empty board follows Existing board below unless initialization is explicitly authorized.
...
2. Create `.taskboard/pending/<session-label>.md` using a filesystem-safe unique session label. Before creating, check for existing pending files. Keep one file per writer and binding; qualify the filename when a session uses multiple boards. Preserve other sessions’ records. Reconcile only records matching the effective project/server/board identity; unresolved identities require clarification. Use the task and handoff templates, including project/server/board identity, known card IDs or explicit local-only identifiers, scope, owner, intended status, evidence, next action, and any uncertain remote operation. Save material checkpoints here while offline.
