...
It does NOT drive a coding agent. The agent is the orchestrator; this file provides
...
Consider 3-4 potential research paths, evaluate them, then select the best strategy.
""",
...
2. Use the available web-fetch tool to inspect 5-10 promising sources
...
- Web fetch: For detailed extraction from specific URLs
- File search and read: For local documentation if relevant
- Subagents: Delegate 2-3 retrieval tasks in parallel when the agent supports subagents
...
"""
    ...
    def execute_phase(self, phase: ResearchPhase) -> Dict[str, Any]:
        ...
        # In real usage, the coding agent executes these instructions.
...
Examples:
  ...
  python research_engine.py -q "longevity biotech funding trends" -m ultradeep
    ...
    print(f"\nNow use your coding agent to execute each phase using the displayed instructions.")
