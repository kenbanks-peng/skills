...
written to file immediately using the current agent's file-edit capability.
...
AGENT OUTPUT LIMIT: Varies by agent. Keep each generated section within the current agent's limit.
For reports that exceed one run: use the continuation protocol for separate batches.
