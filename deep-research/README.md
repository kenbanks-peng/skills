...
Agent-agnostic research skill for coding agents. Produces citation-backed reports with source credibility scoring, multi-provider search, and automated validation.

## Installation

```bash
# Install with the standard agent-skills CLI
npx skills@latest add kenbanks-peng/deep-research-skill --yes
```

The installer detects supported coding agents and installs the skill in their expected location.

For manual or development installation, copy the repository into a skill location that supports `SKILL.md` files.
...
| Agent web-search tool | Fallback — if search-cli fails or is rate-limited | Provided by some agents |
| Exa or another semantic-search integration | Optional — semantic/neural search alongside search-cli | Agent-specific configuration |
