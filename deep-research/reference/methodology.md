...
**Extended reasoning:** Explore multiple framings of the question before committing to scope.
...
**Planning approach:** Consider multiple research paths, then select the best-supported strategy.
...
Before any searches, retrieve today's date with an available shell or date capability: `date +%Y-%m-%d`
...
- Run through the agent's shell capability: `search "query" --json -c 10`

**Fallback: agent web search (if search-cli fails or is unavailable)**
- Use the web-search capability provided by the current agent.
- Use it when search-cli returns errors, is rate-limited, or domain filtering is required.

**Optional: semantic-search integration (if configured)**
- Use Exa or an equivalent semantic-search provider alongside search-cli keyword results.
- Follow the current agent's tool syntax and configuration.
...
When the current agent supports subagents, delegate 3-5 focused tasks for:
...
```
Run these independent searches concurrently when the agent supports parallel tools:
- search "quantum computing [CURRENT_YEAR] state of the art" --json -c 10
- search "quantum computing limitations challenges" --json -c 10
- search "quantum computing commercial applications [CURRENT_YEAR]" -m news --json -c 10
- search "quantum computing vs classical comparison" --json -c 10
- search "quantum error correction research" -m academic --json -c 10

Delegate focused investigations when subagents are available:
- Analyze recent quantum-computing papers. Extract findings and methods.
- Analyze industry reports and market data. Identify commercial applications.
- Extract technical limitations and challenges from primary sources.
```

Use the current agent's tool syntax for web, semantic-search, and subagent capabilities.
...
- Fall back to the agent's web-search capability if search-cli fails or is rate-limited
- Use an available web-fetch capability for deep dives into specific sources
- Use Exa or another semantic-search integration when available
- Use file search and read tools for local documentation
- Execute code for computational analysis when needed
- Delegate parallel retrieval tasks when the agent supports subagents
...
**Extended reasoning:** Explore non-obvious connections and second-order implications.
...
Delegate tasks to subagents when the current agent supports them:
