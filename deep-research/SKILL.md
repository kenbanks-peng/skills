...
**Portability:** Map these instructions to the current agent's equivalent capabilities. Use web search, web fetch, shell, file editing, parallel execution, and subagents only when they are available. Continue without optional capabilities when they are unavailable.
...
```
Request Analysis
+-- Simple lookup? --> STOP: Use the agent's web-search tool
...
**Output files (all to `research/[Topic]_Research_[YYYYMMDD]/` in this skill repository):**
